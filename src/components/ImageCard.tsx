...
export default function ImageCard(props: Property) {
  return (
    <article className="w-full min-w-58 flex flex-col gap-2">
      <div className="relative">
        {/* 이미지 영역 */}
        <img
          src={props.imgSrc}
          className="w-58 aspect-3/4 bg-neutral-400 rounded-xl object-cover"
        />

        <header className="w-full flex flex-row justify-between items-end absolute bottom-0 p-2 z-10">
          <h3 className="w-3/4 font-bold text-2xl tracking-tight line-clamp-3">
            {props.title}
          </h3>
